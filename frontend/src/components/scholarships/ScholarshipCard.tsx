...
import { NotStated } from '../common/States';
import {
  AlertCircle,
  AlertTriangle,
  Bookmark,
...
export const ScholarshipCard: React.FC<ScholarshipCardProps> = ({ scholarship: s, onToggleSave, isSaving = false }) => {
  ...
  const matchBadge =
    matchScore !== undefined && matchScore !== null ? getEligibilityBadgeStyle(eligibilityStatus, matchScore) : null;
  const MatchBadgeIcon = matchBadge?.icon || HelpCircle;
    ...
    <div className="group relative rounded-2xl glass-card border border-dark-border hover:border-brand-500/50 hover:shadow-xl hover:shadow-brand-500/5 transition-all duration-300 flex flex-col justify-between overflow-hidden bg-gradient-to-b from-dark-card/90 to-dark-bg/95">
      ...
      <div
        className={`h-1 w-full transition-all duration-300 ${
          matchScore && matchScore >= 85 ? 'bg-emerald-500' : 'bg-dark-border group-hover:bg-brand-500'
        }`}
      />
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {s.isDemo && (
              <span
                className="px-2 py-0.5 rounded-md text-2xs font-extrabold uppercase tracking-wider bg-amber-500/20 text-amber-300 border border-amber-500/40 shadow-sm"
                title="Demonstration & testing record"
              >
                DEMO DATA
              </span>
            )}
            ...
            <span
              className={`px-2 py-0.5 rounded-md text-2xs font-bold flex items-center gap-1 border ${getStatusBadge(s.verificationStatus || 'VERIFIED')}`}
            >
            ...
            {matchBadge && (
              <button
                ...
                className={`px-2 py-0.5 rounded-md text-2xs font-bold flex items-center gap-1 border transition-all cursor-pointer shadow-sm ${matchBadge.classes}`}
              >
            ...
            {s.applicationStatus && (
              <span className="px-2 py-0.5 rounded-md text-2xs font-semibold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
        ...
        <div className="p-3 rounded-xl bg-dark-bg/70 border border-dark-border/60 space-y-2 text-xs">
          <div className="flex items-center justify-between">
            <span className="px-2 py-0.5 rounded text-2xs font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 uppercase">
              {fundingLabel}
            </span>
            {/* Tuition coverage is shown only when the record actually states it. The
                fallbacks here previously asserted "100% Tuition Waiver" for a null value. */}
            {s.tuitionCoverage ? (
              <span
                className="font-semibold text-emerald-400 text-2xs truncate max-w-[170px]"
                title={s.tuitionCoverage}
              >
                {s.tuitionCoverage}
              </span>
            ) : (
              <NotStated className="text-2xs" label="Tuition not stated" />
            )}
          </div>

          <div className="flex flex-wrap gap-1.5 pt-1 border-t border-dark-border/40 text-2xs text-slate-300">
            {s.stipendAmount && (
        ...
        <div className="space-y-1.5">
          <div className="flex flex-wrap gap-1">
            {s.degreeLevels?.slice(0, 3).map((deg, idx) => (
              <span
                key={idx}
                className="px-2 py-0.5 rounded text-2xs font-medium bg-slate-800/80 text-slate-300 border border-slate-700/60"
              >
                {deg}
              </span>
            ))}
            {s.fieldsOfStudy?.slice(0, 2).map((field, idx) => (
              <span
                key={idx}
                className="px-2 py-0.5 rounded text-2xs font-medium bg-brand-950/60 text-brand-300 border border-brand-800/40 truncate max-w-[130px]"
                title={field}
              >
                {field}
              </span>
            ...
            {(s.fieldsOfStudy?.length || 0) > 2 && (
              <span className="px-1.5 py-0.5 rounded text-2xs text-slate-500">
      ...
      <div className="px-5 py-3.5 bg-dark-card/90 border-t border-dark-border/60 flex items-center justify-between gap-3 text-xs">
        ...
        <div className="space-y-0.5">
          <div className="text-2xs text-slate-400 font-medium">Application Deadline</div>
          <div className="flex items-center gap-1.5 font-bold">
            ...
            {daysRemaining !== null && !isExpired && (
              <span className="text-2xs font-normal text-slate-400">({daysRemaining}d left)</span>
            )}
          </div>
        </div>
        ...
        <Link
          to={`/scholarships/${s.id}`}
          className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-semibold text-xs transition group-hover:translate-x-0.5 duration-200 shrink-0"
        >
