      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Extract academic credentials, verify clarity, identify missing details, and evaluate international
            ...
            {history.map((item, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xs text-slate-400">{new Date(item.createdAt).toLocaleDateString()}</span>
                </div>
                <div className="text-2xs text-slate-300 line-clamp-1">
                  Skills:{' '}
                  {Array.isArray(item.skillsFound) ? item.skillsFound.slice(0, 3).join(', ') : 'Technical Skills'}
                </div>
                {item.id && (
                  <button
        ...
        <div className="lg:col-span-5 p-6 rounded-3xl glass-panel border border-dark-border space-y-5 shadow-xl">
          ...
          <form onSubmit={handleAnalyze} className="space-y-4 text-xs">
            {activeTab === 'upload' ? (
              <div className="space-y-3">
                <div className="border-2 border-dashed border-dark-border hover:border-brand-500/50 rounded-2xl p-6 text-center transition bg-dark-bg/40">
                  ...
                  <p className="text-2xs text-slate-400 mb-3">Supported formats: PDF, DOCX, or TXT (Max size: 5MB)</p>
            ...
            <div className="p-3 rounded-2xl bg-dark-bg/60 border border-dark-border/60 text-2xs text-slate-400 space-y-1">
              ...
              <p>
                Our AI evaluator analyzes verified qualifications and provides objective feedback. It never invents,
                hallucinates, or exaggerates achievements.
              </p>
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 disabled:opacity-50 text-white font-bold flex items-center justify-center gap-2 transition"
            >
              {loading ? (
                <>
          ...
          {!analysis ? (
            <div className="p-12 text-center glass-card rounded-3xl border border-dark-border text-slate-400 space-y-3 min-h-[440px] flex flex-col items-center justify-center shadow-xl">
              ...
              <p className="text-xs max-w-sm mx-auto">
                Upload or paste your academic CV to inspect verified entities across education, skills, projects,
                research, clarity, and missing committee requirements.
              </p>
            </div>
          ) : (
            <div className="p-6 rounded-3xl glass-panel border border-emerald-500/30 space-y-6 shadow-2xl animate-in fade-in">
              ...
              <div className="flex items-center justify-between border-b border-dark-border pb-4 gap-4 flex-wrap">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-2xs font-extrabold border ${getScoreRating(analysis.score).badge}`}
                    >
                ...
                <div className="text-right flex items-center gap-3">
                  <div>
                    ...
                    <div className="text-2xs text-slate-400 font-semibold uppercase">Overall Quality</div>
                  </div>
                </div>
              </div>
              ...
              {analysis.extractedEntities && (
                <div className="space-y-3 text-xs pt-2 border-t border-dark-border">
                  <div className="flex items-center justify-between">
                    ...
                    {analysis.extractedEntities.skills?.length > 0 && (
                      <button
                        ...
                        disabled={syncing}
                        className="px-3 py-1 rounded-xl bg-indigo-500/20 hover:bg-indigo-500 hover:text-white border border-indigo-500/30 text-indigo-300 text-2xs font-semibold flex items-center gap-1.5 transition"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    {analysis.extractedEntities.education?.length > 0 && (
                      <div className="p-3 rounded-2xl bg-dark-card/60 border border-dark-border space-y-1">
                        <span className="text-2xs font-bold text-indigo-300 flex items-center gap-1">
                          <GraduationCap className="w-3.5 h-3.5" /> Education Found:
                        </span>
                        <ul className="space-y-0.5 text-slate-300 text-2xs">
                    ...
                    {analysis.extractedEntities.experience && analysis.extractedEntities.experience.length > 0 && (
                      <div className="p-3 rounded-2xl bg-dark-card/60 border border-dark-border space-y-1">
                        <span className="text-2xs font-bold text-cyan-300 flex items-center gap-1">
                    ...
                    {analysis.extractedEntities.projects && analysis.extractedEntities.projects.length > 0 && (
                      <div className="p-3 rounded-2xl bg-dark-card/60 border border-dark-border space-y-1">
                        <span className="text-2xs font-bold text-amber-300 flex items-center gap-1">
                    ...
                    {analysis.extractedEntities.research && analysis.extractedEntities.research.length > 0 && (
                      <div className="p-3 rounded-2xl bg-dark-card/60 border border-dark-border space-y-1">
                        <span className="text-2xs font-bold text-emerald-300 flex items-center gap-1">
                  ...
                  {analysis.extractedEntities.skills?.length > 0 && (
                    <div className="pt-2">
                      <span className="text-2xs text-slate-400 block mb-1.5 font-semibold">
                        Verified Skills Extracted:
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {analysis.extractedEntities.skills.map((s: string, idx: number) => (
                          <span
                            key={idx}
                            className="px-2.5 py-0.5 rounded-lg bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-2xs font-medium"
                          >
                            {s}
                          </span>
                        ))}
                  ...
                  {analysis.extractedEntities.achievements && analysis.extractedEntities.achievements.length > 0 && (
                    <div className="pt-1">
                      <span className="text-2xs text-slate-400 block mb-1 font-semibold">
                        Honors & Achievements (Verified):
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {analysis.extractedEntities.achievements.map((a: string, idx: number) => (
                          <span
                            key={idx}
                            className="px-2.5 py-0.5 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-2xs"
                          >
                            🏆 {a}
                          </span>
                        ))}
                ...
                <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-xs space-y-2">
                  ...
                  <ul className="space-y-1 text-rose-200/90 pl-1">
                    {analysis.missingInformation.map((m: string, idx: number) => (
                      <li key={idx} className="flex items-start gap-1.5">
                        <AlertTriangle className="w-3 h-3 text-amber-400 mt-1 shrink-0" aria-hidden="true" />
                        <span>{m}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-dark-border text-xs">
                ...
                <button
                  onClick={() => navigate('/sop-assistant')}
                  className="px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-bold flex items-center gap-1.5 transition shadow-md"
                >
